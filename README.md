...
命令检查 hook 读取黑名单规则，然后检查每次 Bash 工具调用。命中后会打开一个 macOS 弹窗，窗口里展示项目名、命中的短语和完整命令文本；选择 `Allow` 继续执行，选择 `Deny` 会向 Codex 返回 deny 决策。默认规则来源是 `~/.claude/settings.json` 里的 `permissions.ask`，也可以设置 `CODEX_BLACKLIST_RULES_FILE` 指向普通文本规则文件。
