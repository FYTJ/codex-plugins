...
3. 如果当前目录是 Git 仓库，先读取当前工作区状态，恢复前后都要报告：

```bash
git status --short
```

如果不是 Git 仓库，继续使用 hook 的文件备份记录。
...
其中 `<n>` 是从选中用户消息所在 turn 开始到当前状态为止需要丢弃的 Codex turn 数；它可能大于可见用户消息数，因为中间可能存在没有用户消息的空 turn。工作区回滚使用同一 target 对应的 hook checkpoint，并把 Git 工作区恢复到该用户 prompt 提交前的 tracked/index/untracked 状态。

代码回滚的覆盖范围与 Claude Code 一致地以 Codex 已知的文件修改为核心：hook 能覆盖 `apply_patch`、Write/Edit 以及可解析的 shell 重定向和常见文件命令；App 既有会话的 fallback 覆盖 rollout 里记录的 `apply_patch`。Git 仓库额外保存 prompt 前的 staged/unstaged/untracked snapshot，用于弥补 hook 无法完美追踪所有修改路径的缺口。
...
```bash
~/.codex/scripts/codex_rewind_patch_app.py
```
...
如果恢复后仍有非预期修改，建议用户先查看 `git diff`，再决定是否继续手动处理。
